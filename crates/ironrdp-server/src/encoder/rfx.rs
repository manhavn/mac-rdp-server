...
impl<'a> UpdateEncoder<'a> {
    ...
    fn encode(&self, data: &'a mut UpdateEncoderData) -> EncodeResult<Vec<rfx::Tile<'a>>> {
        ...
        let (tiles_x, tiles_y) = self.tiles_xy();
        let base_tile_x = usize::from(self.bitmap.x / 64);
        let base_tile_y = usize::from(self.bitmap.y / 64);
        ...
        chunks
            ...
            .map(|(buf, (tile_x, tile_y))| {
                ...
                let tile = rfx::Tile {
                    ...
                    cb_quant_index: 0,
                    cr_quant_index: 0,
                    x: cast_int!("tile_x", base_tile_x + tile_x)?,
                    y: cast_int!("tile_y", base_tile_y + tile_y)?,
                    y_data,
                    cb_data,
                    cr_data,
                };
    ...
    fn encode_tile<'b>(
        ...
        // Edge extrapolation for partial tiles (e.g. bottom 56 rows of 1080p screen)
        // This prevents high-frequency cliff-drop DWT ringing / horizontal stripes.
        if tile_width < 64 || tile_height < 64 {
            ...
            for plane in [&mut *y, &mut *cb, &mut *cr] {
                if tw < 64 && tw > 0 {
                    for row in 0..th {
                        let last_val = plane[row * 64 + tw - 1];
                        for col in tw..64 {
                            plane[row * 64 + col] = last_val;
                        }
                    }
                }
                if th < 64 && th > 0 {
                    let src_start = (th - 1) * 64;
                    for row in th..64 {
                        let dst_start = row * 64;
                        for col in 0..64 {
                            plane[dst_start + col] = plane[src_start + col];
                        }
                    }
                }
            }
